    ...
    $.fn.extend({
        setTreeNode : function(rd, row){
            return this.each(function(){
                ...
                var thecell = $("td:eq("+expCol+")",row).dblclick(function(e){
                    var target = e.target || e.srcElement;
                    var rowid = $(target,$t.rows).parent("tr").attr("id");
        			if(!rowid ){
        				rowid = $(target,$t.rows).parents("tr:first").attr("id");
        			}                    
                ...
                $(".treeclick",thecell).click(function(e){
                    var target = e.target || e.srcElement;
                    var rowid = $(target,$t.rows).parents("tr:first").attr("id");
                    var ind = $($t).getInd($t.rows,rowid);
                    if(!$t.rows[ind].isLeaf){
                        if($t.rows[ind].expanded){
                            $($t).collapseRow($t.rows[ind]);
