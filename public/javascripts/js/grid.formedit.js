    ...
    editGridRow : function(rowid, p){
        ...
        return this.each(function(){
                ...
                $("#sData", "#"+frmtb).click(function(e){
                    ...
                    $(".FormElement", "#"+frmtb).each(function(i){
                        ...
                        switch ($(this).get(0).type) {
                            ...
                            case "textarea":
                                postdata[this.name] = $(this).val();
								ret = checkValues($(this).val(),valref[i]);
			...
			function checkValues(val, valref) {
				if(valref >=0)
					var edtrul = $t.p.colModel[valref].editrules;
				if(edtrul) {
					if(edtrul.required == true) {
						if( val.match(/^s+$/) || val == "" )  return [false,$t.p.colNames[valref]+": "+$.jgrid.edit.msg.required,""];
					}
					if(edtrul.number == true) {
						if(isNaN(val)) return [false,$t.p.colNames[valref]+": "+$.jgrid.edit.msg.number,""];
					}
					if(edtrul.minValue && !isNaN(edtrul.minValue)) {
						if (parseFloat(val) < parseFloat(edtrul.minValue) ) return [false,$t.p.colNames[valref]+": "+$.jgrid.edit.msg.minValue+" "+edtrul.minValue,""];
					}
					if(edtrul.maxValue && !isNaN(edtrul.maxValue)) {
						if (parseFloat(val) > parseFloat(edtrul.maxValue) ) return [false,$t.p.colNames[valref]+": "+$.jgrid.edit.msg.maxValue+" "+edtrul.maxValue,""];
					}
					if(edtrul.email == true) {
						...
						if(!filter.test(val)) {return [false,$t.p.colNames[valref]+": "+$.jgrid.edit.msg.email,""];}
					}
				}
				return [true,"",""];
			};
...
function createModal(aIDs, content, p, insertSelector, posSelector, appendsel) {
	...
	if (p.drag) {
		...
		mf = jQuery("<img  class='jqResize' src='"+p.imgpath+"resize.gif'/>");
		jQuery(mw).append(mf);
	}
