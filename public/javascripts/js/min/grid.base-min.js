...
t.p.selrow=$(pt).attr("id");t.p.selindex=$(pt)[0].rowIndex;$(pt).addClass("selected");if(t.p.onSelectRow){t.p.onSelectRow(t.p.selrow,true);}}}else{t.p.selrow=selection;var ia=t.p.selarrrow.indexOf(t.p.selrow);if(ia===-1){if($(pt).attr("class")!=="subgrid"){$(pt).addClass("selected");}
...
success=true;$(this.p.colModel).each(function(i){nm=this.name;if(data[nm]!='undefined'){$("td:eq("+i+")",t.rows[ind]).html(data[nm]);success=true;}});}});return success;};$.fn.addRowData=function(rowid,data,pos){if(!pos){pos="last";}
var success=false;var nm,row,td,gi=0,si=0;if(data){this.each(function(){var t=this;row=document.createElement("tr");row.id=rowid||t.p.records+1;$(row).addClass("jqgrow");if(t.p.multiselect){td=$('<td></td>');$(td[0],t.grid.bDiv).html("<input type='checkbox'"+" id='jqg_"+rowid+"' class='cbox'/>");row.appendChild(td[0]);gi=1;}
...
$(this.p.colModel).each(function(i){if(colname.indexOf(this.name)!=-1&&!this.hidden){var w=parseInt($("tr th:eq("+i+")",$t.grid.hDiv).css("width"),10);$("tr th:eq("+i+")",$t.grid.hDiv).css({display:"none"});$($t.rows).each(function(j){$("td:eq("+i+")",$t.rows[j]).css({display:"none"});});$t.grid.cols[i].style.width=0;$t.grid.headers[i].width=0;$t.grid.width-=w;this.hidden=true;fndh=true;}});if(fndh===true){var gtw=Math.min($t.p.width,$t.grid.width);$("table:first",$t.grid.hDiv).width(gtw);$("table:first",$t.grid.bDiv).width(gtw);$($t.grid.hDiv).width(gtw);$($t.grid.bDiv).width(gtw);if($t.p.pager&&$($t.p.pager).attr("class")=="scroll"){$($t.p.pager).width(gtw);}
...
if(typeof colname=='string'){colname=[colname];}
$($t.p.colModel).each(function(i){if(colname.indexOf(this.name)!=-1&&this.hidden){var w=parseInt($("tr th:eq("+i+")",$t.grid.hDiv).css("width"),10);$("tr th:eq("+i+")",$t.grid.hDiv).css("display","");$($t.rows).each(function(j){$("td:eq("+i+")",$t.rows[j]).css("display","").width(w);});this.hidden=false;$t.grid.cols[i].style.width=w;$t.grid.headers[i].width=w;$t.grid.width+=w;fdns=true;}});if(fdns===true){var gtw=Math.min($t.p.width,$t.grid.width);var ofl=($t.grid.width<=$t.p.width)?"hidden":"auto";$("table:first",$t.grid.hDiv).width(gtw);$("table:first",$t.grid.bDiv).width(gtw);$($t.grid.hDiv).width(gtw);$($t.grid.bDiv).width(gtw).css("overflow-x",ofl);if($t.p.pager&&$($t.p.pager).attr("class")=="scroll"){$($t.p.pager).width(gtw);}
if($t.p.caption){$($t.grid.cDiv).width(gtw);}
if($t.p.toolbar[0]){$($t.grid.uDiv).width(gtw);}
...
$("tbody tr:gt(0)",$t.grid.bDiv).remove();$t.p.selrow=null;$t.p.selarrrow=[];});};$.fn.getInd=function(obj,rowid){var ret=false;$(obj).each(function(i){if(this.id==rowid){ret=i;return false;}});return ret;};return this.each(function(){if(this.grid){return;}
...
if(j%2==1){row.className=cn;}$(row).addClass("jqgrow");$("tbody",t).append(row);if(afterInsRow){ts.p.afterInsertRow(row.id,rd);}
...
var row,cur,gi=0,si=0,drows,idn,rd=[],cn=(ts.p.altRows===true)?'alt':'';ts.p.page=data[ts.p.jsonReader.page];ts.p.lastpage=data[ts.p.jsonReader.total];ts.p.records=data[ts.p.jsonReader.records];ts.p.userData=data[ts.p.jsonReader.userdata]||{};idn=(!ts.p.keyIndex)?ts.p.jsonReader.id:ts.p.keyIndex;if(!ts.p.jsonReader.repeatitems){var f=reader("json");}
drows=data[ts.p.jsonReader.root];if(drows){for(var i=0;i<drows.length;i++){cur=drows[i];row=document.createElement("tr");row.id=cur[idn]||i+1;if(ts.p.multiselect){addMulti(t,row);gi=1;}
if(ts.p.subGrid){$(ts).addSubGrid(t,row,gi);si=1;}
...
if(i%2==1){row.className=cn;}$(row).addClass("jqgrow");$("tbody",t).append(row);if(afterInsRow){ts.p.afterInsertRow(row.id,rd);}
rd=[];}}
...
if(ts.p.pginput===true){$('#sp_1',ts.p.pager).html("/"+"&nbsp;"+ts.p.lastpage);$('input.selbox',ts.p.pager).val(ts.p.page);}
if(ts.p.viewrecords){$('#sp_2',ts.p.pager).html(ts.p.records+"&nbsp;"+ts.p.recordtext+"&nbsp;");}
...
return(xmlDoc&&xmlDoc.documentElement&&xmlDoc.documentElement.tagName!='parsererror')?xmlDoc:null;};var sortArrayData=function(){var newDir=(ts.p.sortorder=="asc")?1:-1;var column=(ts.p.lastsort>=0)?ts.p.lastsort:0;var st=ts.p.colModel[column].sorttype,findSortKey;if(st=='float'){findSortKey=function($cell){var key=parseFloat($cell.html().replace(/,/g,''));return isNaN(key)?0:key;};}else if(st=='int'){findSortKey=function($cell){return IntNum($cell.html().replace(/,/g,''));};}else if(st=='date'){findSortKey=function($cell){var fd=ts.p.colModel[column].datefmt||"Y-m-d";return parseDate(fd,$cell.html()).getTime();};}else{findSortKey=function($cell){return $cell.html().toUpperCase();};}
var rows=$(ts.grid.bDiv).find('tbody > tr:gt(0)').get();$.each(rows,function(index,row){row.sortKey=findSortKey($(row).children('td').eq(column));var a=1;});rows.sort(function(a,b){if(a.sortKey<b.sortKey){return-newDir;}
if(a.sortKey>b.sortKey){return newDir;}
return 0;});$.each(rows,function(index,row){$('tbody',ts.grid.bDiv).append(row);row.sortKey=null;});if(!isMSIE){ts.scrollLeft=ts.scrollLeft;resizeFirstRow(grid.bDiv);}
...
return new Date(tsp.y,tsp.m,tsp.d,tsp.h,tsp.i,tsp.s,0);};var setPager=function(){var inpt="<img class='pgbuttons' src='"+ts.p.imgpath+"spacer.gif'";var pginp=(ts.p.pginput===true)?"<input class='selbox' type='text' size='3' maxlength='5' value='0'/><span id='sp_1'></span>&nbsp":"";var pgl="",pgr="";if(ts.p.pgbuttons===true){pgl=inpt+" id='first'/>&nbsp;&nbsp;"+inpt+" id='prev'/>&nbsp;";pgr=inpt+" id='next' />&nbsp;&nbsp;"+inpt+" id='last'/>";}
...
ts.p.colModel[l].width=Math.round(tblwidth/initwidth*ts.p.colModel[l].width);}};if(this.p.subGrid){this.p.colNames.unshift("");this.p.colModel.unshift({name:'subgrid',width:25,sortable:false,resizable:false});}
...
grid.width=IntNum(gw)-IntNum(hdc);ts.p.width=grid.width;grid.hTable=document.createElement("table");grid.hTable.appendChild(thead);$(grid.hTable).addClass("scroll").attr({cellSpacing:"0",cellPadding:"0",border:"0"}).css({width:grid.width+"px",marginRight:"20px"});grid.hDiv=document.createElement("div");var hg=(ts.p.caption&&ts.p.hiddengrid===true)?true:false;$(grid.hDiv).css({width:grid.width+"px",overflow:"hidden"}).prepend('<div class="loading">'+ts.p.loadtext+'</div>').append(grid.hTable).bind("selectstart",function(){return false;});if(hg){$(grid.hDiv).hide();}
if(ts.p.pager){if(typeof ts.p.pager=="string"){ts.p.pager=$("#"+ts.p.pager);}
if($(ts.p.pager).attr("class")=="scroll"){$(ts.p.pager).css({width:grid.width+"px",overflow:"hidden"}).show();ts.p._height=parseInt($(ts.p.pager).height(),10);if(hg){$(ts.p.pager).hide();}}
setPager();}
...
if(!ts.p.multikey){$(ts).setSelection(false,ptr);}else{if(e[ts.p.multikey]){$(ts).setSelection(false,ptr);}else if(ts.p.multiselect){$("[@id^=jqg_]",ptr).attr("checked",false);}}
...
ts.p._height+=parseInt($(grid.hDiv,ts).height(),10);$(grid.hDiv).mousemove(function(e){grid.dragMove(e.clientX);}).after(grid.bDiv).mouseup(function(e){if(grid.resizing){grid.dragEnd();if(grid.newWidth){var gwdt=(grid.width<ts.p.width)?grid.width:ts.p.width;var overfl=(grid.width<=ts.p.width)?"hidden":"auto";if(ts.p.pager&&$(ts.p.pager).attr("class")=="scroll"){$(ts.p.pager).width(gwdt);}
