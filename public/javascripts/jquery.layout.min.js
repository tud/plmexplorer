...
Layout=window.layout=window.layout||{},Timers=Layout.timers=Layout.timers||{},name="layout_"+o.id+"_"+pane+"_"+action;if(Timers[name])return;else Timers[name]=setTimeout(fn,ms);};var clearTimer=function(pane,action){var
Layout=window.layout=window.layout||{},Timers=Layout.timers=Layout.timers||{},name="layout_"+o.id+"_"+pane+"_"+action;if(Timers[name]){clearTimeout(Timers[name]);delete Timers[name];return true;}else
return false;};var create=function(){var isBodyContainer=false;try{if($Container[0].tagName=="BODY"){isBodyContainer=true;$("html").css({height:"100%",overflow:"hidden"});$("body").css({position:"relative",height:"100%",overflow:"hidden",margin:0,padding:0,border:"none"});}}catch(ex){}initOptions();cDims=getContainerDims();initPanes();initHandles();initResizable();sizeContent("all");$.each(c.borderPanes.split(","),function(i,pane){var s=o[pane];if(s.enableCursorHotkey||s.customHotkey){$(document).keydown(keyDown);return false;}});$(window).resize(function(){var timerID="timerLayout_"+o.id;if(window[timerID])clearTimeout(window[timerID]);window[timerID]=null;if($.browser.msie)window[timerID]=setTimeout(resizeAll,100);else
resizeAll();});};var initOptions=function(){opts=transformData(opts);o.name=opts.name||opts.defaults.name||"";$.each("paneSelector,resizerCursor,customHotkey,name".split(","),function(idx,key){delete opts.defaults[key];});var fx=c.fxDefaults;if(opts.fxDefaults)$.extend(fx,opts.fxDefaults);c.center=$.extend(true,{},c.defaults,c.center);$.extend(o.center,opts.center);var o_Center=$.extend(true,{},o.defaults,opts.defaults,o.center);$.each("paneClass,contentSelector,contentIgnoreSelector,applyDefaultStyles,showOverflowOnHover".split(","),function(idx,key){o.center[key]=o_Center[key];});$.each(c.borderPanes.split(","),function(i,pane){c[pane]=$.extend(true,{},c.defaults,c[pane]);o[pane]=$.extend(true,{},o.defaults,o[pane],opts.defaults,opts[pane]);var s=o[pane];if(!s.paneClass)s.paneClass=defaults.paneClass;if(!s.resizerClass)s.resizerClass=defaults.resizerClass;if(!s.togglerClass)s.togglerClass=defaults.togglerClass;var fxName=s.fxName;if(fxName!="none"){if(!fxName||!$.effects||!$.effects[fxName]||(!fx[fxName]&&!s.fxSettings))s.fxName="none";else if(fx[fxName])s.fxSettings=$.extend({},fx[fxName].all,fx[fxName][pane],s.fxSettings);}});$.extend(o.defaults,opts.defaults);o.id=Math.floor(Math.random()*10000);};var initPanes=function(){$.each(c.allPanes.split(","),function(){var
pane=str(this),s=o[pane],fx=s.fx,dir=c[pane].dir,size=s.size=="auto"||isNaN(s.size)?0:s.size,minSize=s.minSize||1,maxSize=s.maxSize||9999,spacing=s.spacing_open||0,isIE6=($.browser.msie&&$.browser.version<7),CSS={},$P,$C;$Cs[pane]=false;$P=$Ps[pane]=$Container.children(s.paneSelector);if(!$P.length){$Ps[pane]=false;return true;}$P.attr("pane",pane).addClass(s.paneClass+" "+s.paneClass+"-"+pane);if(pane!="center"){s.isClosed=false;s.isSliding=false;s.isResizing=false;s.isHidden=false;s.noRoom=false;c[pane].pins=[];}CSS=$.extend({visibility:"visible",display:"block"},c.defaults.cssReq,c[pane].cssReq);if(s.applyDefaultStyles)$.extend(CSS,c.defaults.cssDef,c[pane].cssDef);$P.css(CSS);CSS={};switch(pane){case"north":CSS.top=cDims.top;CSS.left=cDims.left;CSS.right=cDims.right;break;case"south":CSS.bottom=cDims.bottom;CSS.left=cDims.left;CSS.right=cDims.right;break;case"west":CSS.left=cDims.left;break;case"east":CSS.right=cDims.right;break;case"center":}s.initSize=s.size;if(dir=="horz"){if(size===0||size=="auto"){$P.css({height:"auto"});size=$P.outerHeight();}size=max(size,minSize);size=min(size,maxSize);size=min(size,cDims.innerHeight-spacing);CSS.height=max(1,cssH(pane,size));s.size=size;s.minSize=max(minSize,size-CSS.height+1);$P.css(CSS);}else if(dir=="vert"){if(size===0||size=="auto"){$P.css({width:"auto",float:"left"});size=$P.outerWidth();$P.css({float:"none"});}size=max(size,minSize);size=min(size,maxSize);size=min(size,cDims.innerWidth-spacing);CSS.width=max(1,cssW(pane,size));s.size=size;s.minSize=max(minSize,size-CSS.width+1);$P.css(CSS);sizeMidPanes(pane,null,true);}else if(pane=="center"){$P.css(CSS);sizeMidPanes("center",null,true);}if(s.initClosed&&!s.isClosable){$P.hide().addClass("closed");s.isClosed=true;}else if(s.initHidden||s.initClosed){hide(pane,true);s.isHidden=true;}else
$P.addClass("open");if(s.showOverflowOnHover)$P.hover(allowOverflow,resetOverflow);if(s.contentSelector){$C=$Cs[pane]=$P.children(s.contentSelector+":first");if(!$C.length){$Cs[pane]=false;return true;}$C.css(c.content.cssReq);if(s.applyDefaultStyles)$C.css(c.content.cssDef);$P.css({overflow:"hidden"});}});};var initHandles=function(){$.each(c.borderPanes.split(","),function(){var
...
edge=c[pane].edge,isOpen=$P.is(":visible"),spacing=(isOpen?s.spacing_open:s.spacing_closed),_side="-"+pane,_state=(isOpen?"-open":"-closed"),$R,$T;$R=$Rs[pane]=$("<span></span>");if(isOpen&&s.resizable);else if(!isOpen&&s.slidable)$R.attr("title",s.sliderTip).css("cursor",s.sliderCursor);$R.attr("id",(s.paneSelector.substr(0,1)=="#"?s.paneSelector.substr(1)+"-resizer":"")).attr("resizer",pane).css(c.resizers.cssReq).css(edge,cDims[edge]+getPaneSize(pane)).addClass(rClass+" "+rClass+_side+" "+rClass+_state+" "+rClass+_side+_state).appendTo($Container);if(s.applyDefaultStyles)$R.css(c.resizers.cssDef);if(s.closable){$T=$Ts[pane]=$("<div></div>");$T.attr("id",(s.paneSelector.substr(0,1)=="#"?s.paneSelector.substr(1)+"-toggler":"")).css(c.togglers.cssReq).attr("title",(isOpen?s.togglerTip_open:s.togglerTip_closed)).click(function(evt){toggle(pane);evt.stopPropagation();}).mouseover(function(evt){evt.stopPropagation();}).addClass(tClass+" "+tClass+_side+" "+tClass+_state+" "+tClass+_side+_state).appendTo($R);if(s.togglerContent_open)$T.append("<span class='content content_open'>"+s.togglerContent_open+"</span>");if(s.togglerContent_closed)$T.append("<span class='content content_closed'>"+s.togglerContent_closed+"</span>");if(s.applyDefaultStyles)$T.css(c.togglers.cssDef);if(!isOpen)bindStartSlidingEvent(pane,true);}});sizeHandles("all",true);};var initResizable=function(){var
...
$P=$Ps[pane],$R=$Rs[pane],$T=$Ts[pane],s=o[pane],doFX=(s.fxName!="none")&&!s.isClosed,edge=c[pane].edge,rClass=s.resizerClass,tClass=s.togglerClass,_side="-"+pane,_open="-open",_sliding="-sliding",_closed="-closed",cP;if(!$P||(!s.resizable&&!s.closable))return;else if(!force&&s.isClosed)return;if(c.isLayoutBusy){setFlowCallback("close",pane,force);return;}else{cP=c[pane];cP.isMoving=true;c.isLayoutBusy=true;}s.isClosed=true;syncPinBtns(pane,false);if(!s.isSliding)sizeMidPanes(c[pane].dir=="horz"?"all":"center");if($R){$R.css(edge,cDims[edge]).removeClass(rClass+_open+" "+rClass+_side+_open).removeClass(rClass+_sliding+" "+rClass+_side+_sliding).addClass(rClass+_closed+" "+rClass+_side+_closed);if(s.resizable)$R.draggable("disable").css("cursor","default").attr("title","");if($T){$T.removeClass(tClass+_open+" "+tClass+_side+_open).addClass(tClass+_closed+" "+tClass+_side+_closed).attr("title",s.togglerTip_closed);}sizeHandles();}if(doFX){lockPaneForFX(pane,true);$P.hide(s.fxName,s.fxSettings,s.fxSpeed,function(){lockPaneForFX(pane,false);if(!s.isClosed)return;close_2();});}else{$P.hide();close_2();}function close_2(){bindStartSlidingEvent(pane,true);execUserCallback(pane,s.onclose);execFlowCallback(pane);}};var open=function(pane,slide){var
$P=$Ps[pane],$R=$Rs[pane],$T=$Ts[pane],s=o[pane],fx=s.fx,doFX=(s.fxName!="none")&&s.isClosed,edge=c[pane].edge,rClass=s.resizerClass,tClass=s.togglerClass,_side="-"+pane,_open="-open",_closed="-closed",_sliding="-sliding",cP;if(!$P||(!s.resizable&&!s.closable))return;else if(!s.isClosed&&!s.isSliding)return;if(c.isLayoutBusy){setFlowCallback("open",pane,slide);return;}else{cP=c[pane];cP.isMoving=true;c.isLayoutBusy=true;}if(s.isSliding&&!slide)bindStopSlidingEvents(pane,false);s.isClosed=false;s.isHidden=false;var maxSize=getPaneMinMaxSizes(pane).maxSize;if(s.size>maxSize)$P.css(c[pane].sizeType,max(1,cssSize(pane,maxSize)));bindStartSlidingEvent(pane,false);if(doFX){lockPaneForFX(pane,true);$P.show(s.fxName,s.fxSettings,s.fxSpeed,function(){lockPaneForFX(pane,false);if(s.isClosed)return;open_2();});}else{$P.show();open_2();}function open_2(){if(!s.isSliding)sizeMidPanes(c[pane].dir=="vert"?"center":"all");if($R){$R.css(edge,cDims[edge]+getPaneSize(pane)).removeClass(rClass+_closed+" "+rClass+_side+_closed).addClass(rClass+_open+" "+rClass+_side+_open).addClass(!s.isSliding?"":rClass+_sliding+" "+rClass+_side+_sliding);if(s.resizable)$R.draggable("enable").css("cursor",s.resizerCursor).attr("title",s.resizerTip);else
$R.css("cursor","default");if($T){$T.removeClass(tClass+_closed+" "+tClass+_side+_closed).addClass(tClass+_open+" "+tClass+_side+_open).attr("title",s.togglerTip_open);}sizeHandles("all");}sizeContent(pane);syncPinBtns(pane,!s.isSliding);execUserCallback(pane,s.onopen);execFlowCallback(pane);}};var lockPaneForFX=function(pane,doLock){var $P=$Ps[pane];if(doLock){$P.css({zIndex:c.zIndex.animation});if(pane=="south")$P.css({top:cDims.top+cDims.innerHeight-$P.outerHeight()});else if(pane=="east")$P.css({left:cDims.left+cDims.innerWidth-$P.outerWidth()});}else{if(!o[pane].isSliding)$P.css({zIndex:c.zIndex.pane_normal});if(pane=="south")$P.css({top:"auto"});else if(pane=="east")$P.css({left:"auto"});}};var bindStartSlidingEvent=function(pane,enable){var
...
edge=c[pane].edge,dir=c[pane].dir,s=o[pane],$P=$Ps[pane],$R=$Rs[pane],limits=getPaneMinMaxSizes(pane);size=max(size,limits.minSize);size=min(size,limits.maxSize);if(s.maxSize>0)size=min(s.maxSize,size);if(s.minSize>0)size=max(s.minSize,size);$R.css(edge,size+cDims[edge]);$P.css(c[pane].sizeType,max(1,cssSize(pane,size)));if(!s.isSliding)sizeMidPanes(dir=="horz"?"all":"center");sizeHandles();sizeContent(pane);s.size=size;execUserCallback(pane,s.onresize);};var sizeMidPanes=function(panes,overrideDims,onInit){if(!panes||panes=="all")panes="east,west,center";var dims=getPaneDims();if(overrideDims)$.extend(dims,overrideDims);$.each(panes.split(","),function(){if(!$Ps[this])return;var
pane=str(this),s=o[pane],$P=$Ps[pane],$R=$Rs[pane],hasRoom=true,CSS={};if(pane=="center"){dims=getPaneDims();CSS=$.extend({},dims);CSS.width=max(1,cssW(pane,CSS.width));CSS.height=max(1,cssH(pane,CSS.height));hasRoom=(CSS.width>1&&CSS.height>1);if($.browser.msie&&(!$.boxModel||$.browser.version<7)){$Ps.north.css({width:cssW($Ps.north,cDims.innerWidth)});$Ps.south.css({width:cssW($Ps.south,cDims.innerWidth)});}}else{CSS.top=dims.top;CSS.bottom=dims.bottom;CSS.height=max(1,cssH(pane,dims.height));hasRoom=(CSS.height>1);}if(hasRoom){$P.css(CSS);if(s.noRoom){s.noRoom=false;if(s.isHidden)return;if(!s.isClosed)$P.show();if($R)$R.show();}if(!onInit){sizeContent(pane);execUserCallback(pane,s.onresize);}}else if(!s.noRoom){s.noRoom=true;if(s.isHidden)return;$P.hide();if($R)$R.hide();}});};var sizeContent=function(panes){if(!panes||panes=="all")panes=c.allPanes;$.each(panes.split(","),function(){if(!$Cs[this])return;var
pane=str(this),ignore=o[pane].contentIgnoreSelector,$P=$Ps[pane],$C=$Cs[pane],e_C=$C[0],height=cssH($P);;$P.children().each(function(){if(this==e_C)return;var $E=$(this);if(!ignore||!$E.is(ignore))height-=$E.outerHeight();});if(height>0)height=cssH($C,height);if(height<1)$C.hide();else
...
$T.show();if(!(togLen>0)||togLen=="100%"||togLen>paneLen){togLen=paneLen;offset=0;}else{if(typeof togAlign=="string"){switch(togAlign){case"top":case"left":offset=0;break;case"bottom":case"right":offset=paneLen-togLen;break;case"middle":case"center":default:offset=Math.floor((paneLen-togLen)/2);}}else{var x=parseInt(togAlign);if(togAlign>=0)offset=x;else offset=paneLen-togLen+x;}}var $TC=(s["togglerContent"+state]?$T.children(".content_"+(s.isClosed?"closed":"open")):false);if(dir=="horz"){var width=cssW($T,togLen);$T.css({width:max(0,width),height:max(1,cssH($T,spacing)),left:offset});if($TC){offset=Math.floor((width-$TC.outerWidth())/2);$TC.css("marginLeft",(offset>0?offset:0));}}else{var height=cssH($T,togLen);$T.css({height:max(0,height),width:max(1,cssW($T,spacing)),top:offset});if($TC){offset=Math.floor((height-$TC.outerHeight())/2);$TC.css("marginTop",(offset>0?offset:0));}}}if(onInit&&s.initHidden){$R.hide();if($T)$T.hide();}});};var resizeAll=function(){var
oldW=cDims.innerWidth,oldH=cDims.innerHeight,d=cDims=getContainerDims(),checkH=(d.innerHeight<oldH),checkW=(d.innerWidth<oldW),s,dir,maxSize;if(checkH||checkW)$.each(["south","north","east","west"],function(i,pane){s=o[pane];dir=c[pane].dir;if(!s.isClosed&&((checkH&&dir=="horz")||(checkW&&dir=="vert"))){maxSize=getPaneMinMaxSizes(pane).maxSize;if(s.size>maxSize)sizePane(pane,maxSize);}});sizeMidPanes("all");sizeHandles("all");};function keyDown(evt){if(!evt)return true;var code=evt.keyCode;if(code<33)return true;var
PANE={38:"north",40:"south",37:"west",39:"east"},isCursorKey=(code>=37&&code<=40),ALT=evt.altKey,SHIFT=evt.shiftKey,CTRL=evt.ctrlKey,pane=false,s,k,m,el;if(!CTRL&&!SHIFT)return true;else if(isCursorKey&&o[PANE[code]].enableCursorHotkey)pane=PANE[code];else
$.each(c.borderPanes.split(","),function(i,p){s=o[p];k=s.customHotkey;m=s.customHotkeyModifier;if((SHIFT&&m=="SHIFT")||(CTRL&&m=="CTRL")||(CTRL&&SHIFT)){if(k&&code==(isNaN(k)||k<=9?k.toUpperCase().charCodeAt(0):k)){pane=p;return false;}}});if(!pane)return true;s=o[pane];if(!s.enableCursorHotkey||s.isHidden||!$Ps[pane])return true;el=evt.target||evt.srcElement;if(el&&SHIFT&&isCursorKey&&(el.tagName=="TEXTAREA"||(el.tagName=="INPUT"&&(code==37||code==39))))return true;toggle(pane);evt.stopPropagation();evt.returnValue=false;return false;};function allowOverflow(elem){if(this&&this.tagName)elem=this;var $P;if(typeof elem=="string")$P=$Ps[elem];else{if($(elem).attr("pane"))$P=$(elem);else $P=$(elem).parents("div[pane]:first");}if(!$P.length)return;var
...
newCSS={zIndex:(c.zIndex.pane_normal+1)},curCSS={},of=$P.css("overflow"),ofX=$P.css("overflowX"),ofY=$P.css("overflowY");if(of!="visible"){curCSS.overflow=of;newCSS.overflow="visible";}if(ofX&&ofX!="visible"&&ofX!="auto"){curCSS.overflowX=ofX;newCSS.overflowX="visible";}if(ofY&&ofY!="visible"&&ofY!="auto"){curCSS.overflowY=ofX;newCSS.overflowY="visible";}c[pane].cssSaved=curCSS;$P.css(newCSS);$.each(c.allPanes.split(","),function(i,p){if(p!=pane)resetOverflow(p);});};function resetOverflow(elem){if(this&&this.tagName)elem=this;var $P;if(typeof elem=="string")$P=$Ps[elem];else{if($(elem).hasClass("ui-layout-pane"))$P=$(elem);else $P=$(elem).parents("div[pane]:first");}if(!$P.length)return;var
